...
namespace HostSeeker
{
    public partial class Form1 : Form
    {
        ...
        private void GetLocalNetworkInterface()
        {
            ...
            interfaceBox.Items.Add("<Choose Network Interface>");
            ...
            foreach (NetworkInterface Networkcards in MyInterfaces)
            {
                ...
                if (anyCast.Count > 0)
                {
                    foreach (IPAddressInformation any in anyCast)
                    {
                        ...
                        {
                            interfaceBox.Items.Add(Networkcards.Name);
                            m_lLocalIP.Add(any);
        ...
        private void GetLocalIP_Click(object oSender, EventArgs cEv)
        {
            try
            {
                ...
                sIP = ((IPAddressInformation)m_lLocalIP[interfaceBox.SelectedIndex - 1]).Address.ToString();
        ...
        {
            if (interfaceBox.SelectedIndex > 0)
            {
                GetLocalIP.Enabled = true;
            }
            else
            {
                GetLocalIP.Enabled = false;
            }
        }
            ...
            if (CheckSubnetIP())
            {
                ...
                for (int i = m_iMinIPSup; i <= m_iMaxIPSup; i++)
                {
                    ...
                    foreach (CHost cHost in m_lHostList)
                    {
                        ...
                        if ((cHost != null) && (i == cHost.getSuf()))
                        {
                            int iHostRow = hostTable.Rows.Add();

                            // IP Adresse in Hostliste eintragen...
                            hostTable.Rows[iHostRow].Cells[0].Value = cHost.getAdress();

                            // ... prüfen ob auch ein Host Eintrag existiert...
                            if (cHost.getHostName().Length > 0)
                            {
                                // ... als neuen Knoten eintragen
                                hostTable.Rows[iHostRow].Cells[1].Value = cHost.getHostName();
                            }
                            iNodeCount++;
                ...
                statusBarLabel1.Text = "Finished and " + iNodeCount.ToString() + " Hosts found on this Subnet";
                statusBar.Refresh();
        ...
        private void hostTable_CellContentClick(object oSender, DataGridViewCellEventArgs cEv)
        {
            if (cEv.RowIndex >= 0)
            {
                hostEntry.Items.Clear();
                ...
                foreach (CHost cHost in m_lHostList)
                {
                    ...
                    if (cHost.getAdress().CompareTo(hostTable.Rows[cEv.RowIndex].Cells[0].Value.ToString()) == 0)
                    {
        ...
        private string hostList4Print(bool bSelection)
        {
            ...
            if (bSelection)
            {
                ...
                for (int iTablePos = 0; iTablePos < (hostTable.Rows.Count); iTablePos++)
                {
                    if ((hostTable.Rows[iTablePos].Cells[0].Selected) || (hostTable.Rows[iTablePos].Cells[1].Selected))
                    {
                        // Iteration durch Adressliste
                        foreach (CHost cHost in m_lHostList)
                        {
                            if (cHost.getAdress().CompareTo(hostTable.Rows[iTablePos].Cells[0].Value.ToString()) == 0)
                            {
                                // IP Adresse in Hostliste eintragen...
